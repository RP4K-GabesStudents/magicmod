...
public class DomTomb extends Structure {
    ...
    protected DomTomb(StructureSettings config, Holder<StructureTemplatePool> startpool, Optional<ResourceLocation> jigsaw_origin, int size, HeightProvider start_height, Optional<Heightmap.Types> project_start_height_map, int max_distance_from_center) {
        super(config);
        STARTPOOL = startpool;
    ...
    public static final Codec<DomTomb> CODEC = RecordCodecBuilder.<DomTomb>mapCodec(objectInstance ->
            objectInstance.group(DomTomb.settingsCodec(objectInstance),
            ...
            ).apply(objectInstance, DomTomb::new)
    ).codec();
    ...
    public StructureType<?> type() {
        return ModStructures.DOM_TOMB.get();
    }
