...
public class SurfaceStation extends Structure {
    ...
    protected SurfaceStation(StructureSettings config, Holder<StructureTemplatePool> startpool, Optional<ResourceLocation> jigsaw_origin, int size, HeightProvider start_height, Optional<Heightmap.Types> project_start_height_map, int max_distance_from_center) {
        super(config);
        STARTPOOL = startpool;
    ...
    public static final Codec<SurfaceStation> CODEC = RecordCodecBuilder.<SurfaceStation>mapCodec(objectInstance ->
            objectInstance.group(SurfaceStation.settingsCodec(objectInstance),
            ...
            ).apply(objectInstance, SurfaceStation::new)
    ).codec();
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        if (!SurfaceStation.extraSpawnChecks(context))
    ...
    public StructureType<?> type() {
        return ModStructures.SURFACE_STATION.get();
    }
