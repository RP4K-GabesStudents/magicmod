...
public class EndCastle extends Structure {
    ...
    protected EndCastle(StructureSettings config, Holder<StructureTemplatePool> startpool, Optional<ResourceLocation> jigsaw_origin, int size, HeightProvider start_height, Optional<Heightmap.Types> project_start_height_map, int max_distance_from_center) {
        super(config);
        STARTPOOL = startpool;
    ...
    public static final Codec<EndCastle> CODEC = RecordCodecBuilder.<EndCastle>mapCodec(objectInstance ->
            objectInstance.group(EndCastle.settingsCodec(objectInstance),
                    StructureTemplatePool.CODEC.fieldOf("start_pool").forGetter(structure -> structure.STARTPOOL),
                    ResourceLocation.CODEC.optionalFieldOf("start_jigsaw_name").forGetter(structure -> structure.JIGSAW_ORIGIN),
                    Codec.intRange(1, 6).fieldOf("size").forGetter(structure -> structure.SIZE),
                    ...
                    Codec.intRange(5, 8).fieldOf("max_distance_from_center").forGetter(structure -> structure.MAX_DISTANCE_FROM_CENTER)
            ).apply(objectInstance, EndCastle::new)
    ).codec();

    private static boolean extraSpawnChecks(Structure.GenerationContext context){
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        if (!EndCastle.extraSpawnChecks(context))
        ...
        Optional<Structure.GenerationStub>structurePiecesGenerator = JigsawPlacement.addPieces(context, this.STARTPOOL, this.JIGSAW_ORIGIN, this.SIZE, blockpos, false, this.PROJECT_START_HEIGHT_MAP, this.MAX_DISTANCE_FROM_CENTER);
        return structurePiecesGenerator;
    }

    @Override
    public StructureType<?> type() {
        return ModStructures.END_CASTLE.get();
    }
