...
public class ArcherTower extends Structure {
    ...
    protected ArcherTower(StructureSettings config, Holder<StructureTemplatePool> startpool, Optional<ResourceLocation> jigsaw_origin, int size, HeightProvider start_height, Optional<Heightmap.Types> project_start_height_map, int max_distance_from_center) {
        super(config);
        STARTPOOL = startpool;
    ...
    public static final Codec<ArcherTower> CODEC = RecordCodecBuilder.<ArcherTower>mapCodec(objectInstance ->
            objectInstance.group(ArcherTower.settingsCodec(objectInstance),
            ...
            ).apply(objectInstance, ArcherTower::new)
    ).codec();


    @Override
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        if (!ArcherTower.extraSpawnChecks(context))
    ...
    public StructureType<?> type() {
        return ModStructures.ARCHER_TOWER.get();
    }
