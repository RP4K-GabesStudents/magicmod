...
public class ManaTemple extends Structure {
    ...
    protected ManaTemple(StructureSettings config, Holder<StructureTemplatePool> startpool, Optional<ResourceLocation> jigsaw_origin, int size, HeightProvider start_height, Optional<Heightmap.Types> project_start_height_map, int max_distance_from_center) {
        super(config);
        STARTPOOL = startpool;
    ...
    public static final Codec<ManaTemple> CODEC = RecordCodecBuilder.<ManaTemple>mapCodec(objectInstance ->
            objectInstance.group(ManaTemple.settingsCodec(objectInstance),
                    ...
                    Codec.intRange(0, 1).fieldOf("max_distance_from_center").forGetter(structure -> structure.MAX_DISTANCE_FROM_CENTER)
            ).apply(objectInstance, ManaTemple::new)
    ).codec();
    ...
    public StructureType<?> type() {
        return ModStructures.MANA_TEMPLE.get();
    }
