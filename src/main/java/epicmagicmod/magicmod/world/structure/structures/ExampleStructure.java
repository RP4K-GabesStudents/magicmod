...
public class ExampleStructure extends Structure {
    ...
    protected ExampleStructure(StructureSettings config, Holder<StructureTemplatePool> startpool, Optional<ResourceLocation> jigsaw_origin, int size, HeightProvider start_height, Optional<Heightmap.Types> project_start_height_map, int max_distance_from_center) {
        super(config);
        STARTPOOL = startpool;
    ...
    public static final Codec<ExampleStructure>CODEC = RecordCodecBuilder.<ExampleStructure>mapCodec(objectInstance ->
            objectInstance.group(ExampleStructure.settingsCodec(objectInstance),
                    ...
                    ).apply(objectInstance, ExampleStructure::new)
            ).codec();
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        if (!ExampleStructure.extraSpawnChecks(context))
