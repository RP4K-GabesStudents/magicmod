...
public class UndergroundStation extends Structure {
    ...
    protected UndergroundStation(StructureSettings config, Holder<StructureTemplatePool> startpool, Optional<ResourceLocation> jigsaw_origin, int size, HeightProvider start_height, Optional<Heightmap.Types> project_start_height_map, int max_distance_from_center) {
        super(config);
        STARTPOOL = startpool;
    ...
    public static final Codec<UndergroundStation> CODEC = RecordCodecBuilder.<UndergroundStation>mapCodec(objectInstance ->
            objectInstance.group(UndergroundStation.settingsCodec(objectInstance),
            ...
            ).apply(objectInstance, UndergroundStation::new)
    ).codec();
    ...
    public Optional<GenerationStub> findGenerationPoint(GenerationContext context) {
        if (!UndergroundStation.extraSpawnChecks(context))
    ...
    public StructureType<?> type() {
        return ModStructures.UNDERGROUND_STATION.get();
    }
